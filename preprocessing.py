...
# select top 0.1%
top = a.head(int(len(a)*(0.01/100)))

# %%
# load csv file to dataframe
df = pd.read_csv('data/tracks.csv', dtype={"id":"string"}, converters={'id_artists':lambda x: np.array(ast.literal_eval(x))})
#%%
...
# %%
# order by year
filtered_by_popular_artists = filtered_by_popular_artists.sort_values(by='year')
# %%
#%%
filtered_by_popular_artists['year'].value_counts(sort=False).plot(kind='bar', figsize=(20, 10))
...
# save to csv
filtered_by_popular_artists.to_csv('data/tracks_filtered.csv', index=False, header=True)
...
# select top 0.1%
#top_jazz_artists = a.head(int(len(jazz_artists)*(50/100)))
#remove artists with popularity < 10
top_jazz_artists = jazz_artists[jazz_artists['popularity'] > 65]
...
filtered_by_jazz_artists['year'].value_counts(sort=False).plot(kind='bar', figsize=(20, 10))

# %%
filtered_by_jazz_artists.to_csv('data/tracks_filtered_jazz.csv', index=False, header=True)
